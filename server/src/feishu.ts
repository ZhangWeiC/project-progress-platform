...
export async function loginWithFeishuCode(code: string, state: string) {
  ...
  return {
    ...createSession({ id: employee.id, name: employee.name, role: employee.role }),
...
function upsertEmployeeFromFeishu(user: FeishuOAuthUser | undefined): LocalEmployee | null {
  ...
  return db.prepare('SELECT id, name, role, department_id FROM employee WHERE id = ?').get(saved.id) as LocalEmployee | null;
}
...
function saveFeishuEmployee(
  ...
  if (existing) {
    db.prepare(
      `UPDATE employee
       SET name = ?, department_id = ?, role = ?, feishu_open_id = ?, feishu_union_id = ?, feishu_user_id = ?, email = ?, mobile = ?, avatar_url = ?, is_active = ?, last_feishu_sync_at = ?
       WHERE id = ?`
    ).run(
      employee.name,
      targetDepartmentId,
  ...
  db.prepare(
    `INSERT INTO employee (id, name, department_id, role, feishu_open_id, feishu_union_id, feishu_user_id, email, mobile, avatar_url, is_active, last_feishu_sync_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    localId,
    employee.name,
...
function findEmployee(employee: ReturnType<typeof normalizeFeishuEmployee>): LocalEmployee | null {
  ...
  for (const [column, value] of queries) {
    if (!value) continue;
    const found = db.prepare(`SELECT id, name, role, department_id FROM employee WHERE ${column} = ?`).get(value) as LocalEmployee | undefined;
    if (found) return found;
  }
  return (db.prepare('SELECT id, name, role, department_id FROM employee WHERE name = ?').get(employee.name) as LocalEmployee | undefined) ?? null;
}
